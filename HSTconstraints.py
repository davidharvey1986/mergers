...
def HSTconstraints(SimNames):
    ...
    crossSection = np.array([0., 0.1, 0.3, 1.0, 2.0, 0.0, 0.0])
    ...
    for i, iSimName in enumerate(SimNames):
        ...
        print(len(iClusterSample.ClusterMass))
    ...
    ax.set_xlim(5e-3,3)
    plt.savefig('HSTconstraints.pdf')
    plt.show()
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        simNames = ['CDM','SIDM0.1','SIDM0.3','SIDM1','vdSIDM','CDM_hi','CDM_low'] 
    else:
        simNames = [sys.argv[1]]

    HSTconstraints(simNames)
